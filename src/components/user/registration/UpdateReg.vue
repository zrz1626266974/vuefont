...
export default {
  ...
  created () {
    ...
    this.getRegById(this.userId, this.acid)
    // console.log(this.acid)
  },
  methods: {
    // 根据 报名表id 获取具体报名表信息
    getRegById (userId, acid) {
      reg.getRegById(userId, acid).then(data => {
        this.one = data.data.data.one
        console.log(`userUpdateReg data == `, data.data.data.one)
        // this.$router.push({name: 'myReg'})
      }).catch(err => {
        console.log(err)
      })
    },
    submitForm () {
      this.$refs.one.validate((valid) => {
        if (valid) {
          reg.updateReg(this.one).then(data => {
            this.$router.push({name: 'myReg'})
          }).catch(err => {
            console.log(err)
          })
